...
export default function ProductPage({
    ...
    const [activeImage, setActiveImage] = useState(selectedVariant.mainImage);
    const addItem = useCartStore((state) => state.addItem);

    // Update active image when variant changes
    const handleVariantChange = (variant: typeof product.variants[0]) => {
        setSelectedVariant(variant);
        setActiveImage(variant.mainImage);
    };

    const images = selectedVariant.galleryImages || [selectedVariant.mainImage, selectedVariant.hoverImage || selectedVariant.mainImage];
                    ...
                    <div className="mb-10">
                        <div className="flex flex-wrap gap-2 mb-6">
                            {product.badges && product.badges.map(badge => (
                                <span key={badge} className="inline-block bg-primary text-white text-[9px] font-black px-3 py-1.5 uppercase tracking-widest shadow-sm">
                                    {badge}
                                </span>
                            ))}
                        </div>
                        <h1 className="font-display text-4xl md:text-5xl font-bold mb-4 tracking-tighter leading-tight uppercase italic">{product.name}</h1>
                        <div className="flex items-center space-x-6">
                            <span className="text-2xl md:text-3xl font-black font-display">${product.price}</span>
                            {product.originalPrice && (
                                <span className="text-lg text-muted-light line-through font-medium">${product.originalPrice}</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-10">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-[11px] font-black uppercase tracking-[0.2em]">Color: <span className="text-muted-light">{selectedVariant.colorName}</span></span>
                            </div>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-[11px] font-black uppercase tracking-[0.2em]">Select Size</span>
                        ...
                        <div className="pt-4">
                            <button
                                ...
                                onClick={() => selectedSize && addItem(product, selectedVariant, selectedSize)}
                                className="w-full bg-primary text-white py-5 text-[11px] font-black uppercase tracking-[0.25em] hover:bg-gray-900 transition-all shadow-2xl disabled:bg-gray-100 disabled:text-gray-400 disabled:shadow-none disabled:cursor-not-allowed flex items-center justify-center space-x-3"
                            >
                        ...
                        <div className="border-t border-gray-100 pt-10 space-y-6">
                            ...
                            <div className="flex justify-between items-center py-2 cursor-pointer font-black uppercase tracking-[0.2em] text-[11px] hover:text-primary/60 transition-colors">
