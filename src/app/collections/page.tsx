        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-16">
                <div>
                    ...
                    <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight mb-4">All Products</h1>
                ...
                <div className="flex items-center space-x-6 mt-8 md:mt-0">
                    ...
                    <button className="md:hidden flex items-center space-x-3 text-[11px] font-bold uppercase tracking-[0.2em] border border-gray-200 px-6 py-4 hover:border-black transition-all">
                    ...
                    <div className="sticky top-36">
                        <div className="mb-12">
                            ...
                            <ul className="space-y-5">
                                {["All", "Leggings", "Sports Bras", "Tops", "Shorts", "Sets", "Hoodies"].map(cat => (
                                    <li key={cat}>
                                        <button
                                            onClick={() => setActiveCategory(cat)}
                                            className={`text-[13px] tracking-wide transition-all uppercase ${activeCategory === cat ? 'font-black text-primary translate-x-1' : 'font-medium text-muted-light hover:text-primary hover:translate-x-1'}`}
                                        >
                                            {cat}
                                        </button>
                                    </li>
                        ...
                        <div>
                            <h3 className="text-[11px] font-black uppercase tracking-[0.3em] mb-8 text-primary">Color</h3>
                            <div className="flex flex-wrap gap-3">
                                {["#000000", "#ffffff", "#4A5568", "#8A9A8B", "#9B2C2C", "#1A2C42", "#f472b6"].map(hex => (
                                    <button
                                        key={hex}
                                        className="w-8 h-8 rounded-full border border-gray-200 hover:ring-2 hover:ring-offset-2 hover:ring-primary transition-all"
                                        style={{ backgroundColor: hex }}
                                    >
                                        <span className="sr-only">Color filter</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <div className="flex-1">
                            ...
                            return (
                                <div key={product.id} className="group relative">
                                    ...
                                    {product.variants.length > 1 && (
                                        <div className="flex space-x-2 mb-4">
                                            {product.variants.slice(0, 4).map(v => (
                                                <div key={v.id} className="w-3.5 h-3.5 rounded-full border border-gray-200" style={{ backgroundColor: v.colorHex }}></div>
                                            ))}
                                    ...
                                    <div className="flex flex-col space-y-1">
                                        <h3 className="text-[14px] font-black tracking-tight leading-tight">
                                            <Link href={`/product/${product.id}`} className="hover:text-primary/60 transition-colors capitalize">
                                                {product.name}
                                            </Link>
                                        </h3>
                                        <div className="flex justify-between items-center pt-1">
                                            <p className="text-[12px] text-muted-light font-medium tracking-wide">{product.variants.length} {product.variants.length === 1 ? 'Color' : 'Colors'}</p>
                                            <div className="text-right">
                                                {product.originalPrice ? (
                                                    <div className="flex items-center space-x-3">
                                                        <p className="text-[14px] font-black font-display text-red-600">${product.price}</p>
                                                        <p className="text-[12px] text-muted-light font-medium line-through">${product.originalPrice}</p>
                                                    </div>
                                                ) : (
                    ...
                    {filteredProducts.length === 0 && (
                        <div className="text-center py-32 border-2 border-dashed border-gray-100 rounded-3xl">
                            <h3 className="text-2xl font-black mb-3 italic">Nothing found</h3>
                            <p className="text-muted-light text-sm max-w-xs mx-auto mb-10">We couldn&apos;t find any products matching your current selection. Try a different category.</p>
                            <button
                                onClick={() => setActiveCategory("All")}
                                className="bg-black text-white text-[10px] font-bold uppercase tracking-[0.2em] px-10 py-5 hover:bg-gray-800 transition-all shadow-lg"
                            >
                                Reset Filters
                            </button>
                        </div>
